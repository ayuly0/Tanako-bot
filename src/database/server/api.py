...
from src.database.engine.legacy import ColumnarDB, Column as EngineColumn, DataType
from src.database.engine.distributed import DistributedColumnarDB, Column as DistributedColumn
from src.database.engine.query import QueryBuilder, query

def Column(name, data_type, **kwargs):
    """Factory function to create the correct Column type"""
    return DistributedColumn(name, data_type, **kwargs)
...
class DatabaseAPIServer:
    ...
    async def initialize(self):
        ...
        if self.cluster_enabled:
            ...
            # Optimized Standalone: Use Distributed Engine with Direct Flush for O(1) writes
            self.distributed_db = DistributedColumnarDB(
                data_dir=self.data_dir,
                node_id=self.node_id,
                host=self.host,
            ...
            await self.distributed_db.initialize()
            self.db = None
            logger.info("Optimized Standalone database initialized (Direct-Flush enabled)")
